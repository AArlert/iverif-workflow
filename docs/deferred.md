...
| Risk-graded card paths — L0 docs/build · L1 TB/sequence/coverage · L2 RTL/SVA/scoreboard · L3 signoff/waiver/spec — letting low-risk cards skip parts of the full chain (external review 2026-07-26 rec 4; note the per-card-type dispatch matrix in `skills/dispatch/` already *is* a graded path — a DE fix card never passes through arch) | process overhead visibly exceeding the work itself on a low-risk card, recorded twice |
