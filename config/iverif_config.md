...
| `key_line_extra` | no | `[]` | **Escape hatch** — extra regexes for evidence key-line extraction. The canon convention needs no config: functional-coverage summary lines tagged `[FCOV_SUMMARY]` (loop/evidence_record.md row 6) are captured out of the box. Use only for further project-invented tags. Invalid regexes are a hard error at registration time |
...
| `fl_schema_enforce` | no | `true` | Validate failure-record detail pages of terminal bugs against `loop/fail/failure_record.md`. Legacy repos with free-form pages set `false` |
